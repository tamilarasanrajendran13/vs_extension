...
You have about 8 looks. Spend them well:
  ...
  - READ THE FILE YOU ARE COPYING. If the pattern says "mirror csv_source.py",
    read csv_source.py. A plan written from a summary of a file is a plan that
    gets the details wrong, and the developer will follow it into the wall.
  ...
  - Read the file you are modifying. You cannot write "add a branch to the
    dispatcher" without knowing what the dispatcher looks like.
  - Do not read what you already know from the index or the patterns.
