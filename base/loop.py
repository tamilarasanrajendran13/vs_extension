...
def load_patterns(cfg: dict, tx, project: str, project_path: Path | None,
    ...
    tools = {
        ...
        "read": lambda paths, start=None, end=None: map_repo.render_files(
            map_repo.read_files(pp, paths, start=start, end=end)),
    }
...
def run_planner(tx, cfg: dict, run_id: str, ticket_id: str, ticket_text: str,
    ...
    def _plan_one(i, role, psay):
        ...
        for pattempt in (1, 2):
            popen = user
            if plan is not None:
                popen += ("\n\n=== YOUR PREVIOUS PLAN FAILED VERIFICATION ===\n"
                          + "\n".join(f"- {v['file'] or '(plan)'}: {v['problem'][:160]}"
                                      for v in violations)
                          + "\nFix ONLY these problems and emit the corrected "
                            "complete plan.")
            try:
                r = agent_loop.run(tx, agent, tools, popen, A.get("max_steps", 8),
    ...
    if len(plans) == 1:
        ...
        if j is None:
            ...
            label = letter
            j["winner"] = label
            winner = plans[planning.LABELS.find(label)]
    ...
    try:
        ...
        for sattempt in (1, 2):
            ...
            if serr:
                suser += ("\n\n=== YOUR PREVIOUS REPLY WAS NOT VALID JSON ===\n"
                          f"{str(serr)[:300]}\nReply with exactly ONE JSON object.")
            reply = tx.chat(agent["model"], system, suser)
            try:
