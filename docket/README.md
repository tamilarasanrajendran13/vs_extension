...
- VSIX (recommended - VS Code validates and manages this format): the
  Distribution Kit ships `docket-0.0.1.vsix` (built from `extension/`
...
- Folder copy (offline/manual alternative, no tools needed): copy the
  ...
  renamed to `docket.docket-0.0.1`
  (`<publisher>.<name>-<version>` from extension/package.json):
  `~/.vscode/extensions/docket.docket-0.0.1` on macOS/Linux,
  `%USERPROFILE%\.vscode\extensions\docket.docket-0.0.1` on Windows. Then
