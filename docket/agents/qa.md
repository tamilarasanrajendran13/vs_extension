...
- name and path (where the acceptance tests will look for it, per PATTERNS -
  under the FIXTURE ROOT named in your input).
...
- The generator automatically PREPENDS deterministic boundary rows to every
  dataset (each column's min/max/midpoint, every choice, both bools, the
  date-span edges, string-length extremes), counted against your row budget.
  Do not spend rows re-requesting boundary values - spend your design on
  distributions and the scenarios instead.
...
Every dataset path MUST be project-root-relative under the FIXTURE ROOT named in your input (default test/fixtures/). Absolute paths, drive letters (C:/...), and '..' are REFUSED by the generator - the fixture will simply not exist and the suite will fail on it.
