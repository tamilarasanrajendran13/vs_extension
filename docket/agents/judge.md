...
WHAT YOU ARE SCORING AGAINST
  ...
  1. DOES IT SATISFY EVERY ACCEPTANCE CRITERION?
     A plan that covers four of five criteria loses to one that covers five,
     however elegant. Do NOT count coverage yourself: the COMPUTED FACTS block
     in your ballot is the authoritative count (counted by code, not claimed
     by any planner), and the harness REJECTS a pick that contradicts it. Use
     the facts block; spend your judgement on the questions below, which only
     you can answer, choosing among the plans the facts block ties.
