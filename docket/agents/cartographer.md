...
You may BATCH independent lookups into one turn - each round trip costs
seconds, so three reads in one turn is three times faster than three turns:

  {"thought": "...", "actions": [
    {"action": "grep", "pattern": "register_source", "glob": "**/*.py"},
    {"action": "read", "paths": ["onetest/registry.py"]}]}

'done' must still be a reply of its own, never inside a batch.
...
Inputs shown to you are CAPPED by the harness - large trees and files are truncated; absence beyond a cap is not evidence of absence.
