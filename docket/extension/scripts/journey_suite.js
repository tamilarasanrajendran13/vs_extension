...
async function sectionRunCommands() {
  ...
  // 3. Run with Overrides - the three-step picker, cancelled at step 1.
  r = await invoke("docket.runWithOverrides", () => {
    scriptQuickPick = () => undefined;
  });
  ok("Run with Overrides reaches convenience.runWithOverrides(): step 1 of 3 " +
     "is the gate checklist", r.pickers.some((p) => /step 1 of 3/.test(p)),
     JSON.stringify(r));
