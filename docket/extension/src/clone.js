...
// config.json is the durable source of truth. This small signal only tells
// already-open UI projections to read it again immediately after a project
// switch; it carries no run or project state of its own.
const projectListeners = new Set();

function onDidChangeProject(listener) {
  projectListeners.add(listener);
  return { dispose: () => projectListeners.delete(listener) };
}

function notifyProjectChanged(name) {
  for (const listener of Array.from(projectListeners)) {
    try { listener(name); } catch (_) { /* one renderer cannot block selection */ }
  }
}
...
async function run() {
  ...
  try {
    ...
    const msg = String(e.message).split(String(url)).join(redact(url));
    out.appendLine(`\nFAILED: ${msg}`);
...
async function select() {
  ...
  notifyProjectChanged(picked.name);
  vscode.window.showInformationMessage(`Docket: active project is now ${picked.name}`);
}

module.exports = {
  run, select, redact, folderNameFrom, onDidChangeProject,
};
