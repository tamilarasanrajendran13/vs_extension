...
def _run(cmd, cwd):
    """PHASE 4 (Mac mission, REL-016): the agent-authored unit test runs
    through the ONE containment authority like every other
    model-influenced command."""
    import containment as _cont
    return _cont.run_contained(cmd, cwd, timeout=900)
...
def import_hint(repo, file_rel):
    """D11 (Mac mission Phase 2): the module import path for the file
    under test, derived from PROJECT METADATA - declared pytest
    'pythonpath' roots (developer._declared_ini_list, real pytest
    precedence), else the src/ layout, else the repo root. Never
    guessed from the file path: src/compare.py under a src-layout
    imports as 'compare', not 'src.compare'."""
    rel = str(file_rel).replace("\\", "/").strip("/")
    roots = []
    try:
        sys.path.insert(0, str(Path(__file__).resolve().parent / "scripts"))
        import developer as _dev
        pp = Path(repo)
        for v in _dev._declared_ini_list(pp, "pythonpath"):
            v = str(v).replace("\\", "/").strip().strip("/")
            if v and v != "." and (pp / v).is_dir() and v not in roots:
                roots.append(v)
    except Exception:
        pass
    if not roots and (Path(repo) / "src").is_dir():
        roots.append("src")
...
def _user_prompt(func, src, det, repo=None):
    hint = ""
    if repo is not None:
        try:
            hint = "\nIMPORT AS: from {} import {}\n".format(
                import_hint(repo, func["file"]), func["name"])
        except Exception:
            hint = ""
    return ("FILE: {}\nFUNCTION: {}  (line {})\nPRIMARY LANGUAGE: {}\n{}\n"
            "Write a focused unit test for this function.\n\n{}"
            ).format(func["file"], func["name"], func["lineno"],
                     (det or {}).get("primary", "python"), hint, src)


def _load_agent(workbench, cfg):
    """Load the unit_tester agent prompt/model the same way every stage
    does. D6 (reliability mission 2026-08-05): NO built-in fallback -
    roster.py's contract is 'a missing agent file must never fall back
    to a built-in default: you would edit the file, see no change, and
    have no idea why'. The old _FALLBACK here silently dropped the
    suspected_bug safeguard and the behaviour-assertion rule, hardcoded
    test/unit against the native-root contract, and stamped the ledger
    unit_tester@0 - indistinguishable from a real unversioned agent."""
    if roster is None:
        raise RuntimeError(
            "roster module unavailable - cannot load agents/{}.md; there "
            "is no built-in fallback, on purpose".format(AGENT_NAME))
    A = roster.load(AGENT_NAME, workbench)
    if agent_memory is not None:
        A = agent_memory.attach(A, AGENT_NAME, cfg.get("_project"),
                                workbench)
    return {"model": A.get("model", "worker"), "prompt": A["prompt"],
            "stamp": (roster.stamp(A) if hasattr(roster, "stamp")
                      else AGENT_NAME + "@?")}
    ...
    for func in pending:
        ...
        try:
            reply = tx.chat(A["model"], A["prompt"],
                            _user_prompt(func, src, det, repo=repo))
...
def _self_test():
    ...
    # D11 (Mac mission Phase 2): the import path is derived from PROJECT
    # METADATA (declared pytest pythonpath > src/-layout > repo root),
    # never guessed from the file path - 'from src.compare import x' is
    # wrong under a src-layout and the discard was silent.
    with tempfile.TemporaryDirectory() as td:
        r1 = Path(td)
        (r1 / "src").mkdir()
        (r1 / "src" / "compare.py").write_text("def f():\n    return 1\n")
        ok("D11: src-layout imports as the package, not 'src.'",
           import_hint(r1, "src/compare.py") == "compare")
        r2 = r1 / "plain"
        (r2 / "lib").mkdir(parents=True)
        (r2 / "lib" / "util.py").write_text("x = 1\n")
        ok("D11: plain layout imports by its path",
           import_hint(r2, "lib/util.py") == "lib.util")
        r3 = r1 / "declared"
        (r3 / "python" / "pkg").mkdir(parents=True)
        (r3 / "pyproject.toml").write_text(
            "[tool.pytest.ini_options]\npythonpath = [\"python\"]\n")
        (r3 / "python" / "pkg" / "mod.py").write_text("y = 2\n")
        ok("D11: declared pythonpath root wins over layout guessing",
           import_hint(r3, "python/pkg/mod.py") == "pkg.mod")
        ok("D11: the prompt carries the computed import path",
           "IMPORT AS: from compare import f" in _user_prompt(
               {"file": "src/compare.py", "name": "f", "lineno": 1},
               "def f(): ...", {"primary": "python"}, repo=r1))
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        (repo / "src" / "m.py").write_text(
            "def keep(a):\n    return a + 1\n\ndef drop(a):\n    return a - 1\n")
        # D6: no built-in fallback prompt exists any more - the fixture
        # workbench points at the REAL agent file (self-test convention).
        (repo / "agents").mkdir()
        _real_agent = Path(__file__).resolve().parent / "agents" / (
            AGENT_NAME + ".md")
        (repo / "agents" / (AGENT_NAME + ".md")).write_text(
            _real_agent.read_text(encoding="utf-8"), encoding="utf-8")
