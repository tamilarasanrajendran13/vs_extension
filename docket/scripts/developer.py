    ...
    if baseline["total"] > 0 and not baseline["ok"]:
        ...
        if not cfg.get("_worker_mode"):  # B1(e): the lead's row is canonical
            ledger.gate(run_id, ticket_id, "unit_tests", "unknown", actor=AGENT_NAME,
                        ...
                        details={"unknown_reason": "unit suite red before any "
                                                   "change - dirty or broken tree",
        ...
        return {"outcome": "unknown",
                ...
                # ENVIRONMENT condition (this run's code never ran) with
                # the explicit class so the failing-test text cannot
                # misclassify it as this run's implementation defect.
                "failure_class": "environment_failure",
                "failure_evidence": develop_failure_evidence(
                    "unit suite red before any change ({} failed of {}) - "
                    "dirty or broken tree".format(baseline["failed"],
                                                  baseline["total"]),
                    baseline),
