...
def tasks_from_plan(plan):
    ...
    for i, st in enumerate(plan.get("steps") or [], 1):
        out.append({
            ...
            "what": st.get("what") or "",
            # cohesive plan-slice id (live run DATACMP-3-0b48b5b6): rides
            # through the partition so atomic contracts are never split
            # across parallel workers
            "slice": st.get("slice") or None,
        })
    return out
...
def partition_by_files(tasks):
    ...
    slice_owner = {}  # plan-slice id -> first task id in the slice
    for t in tasks:
        # A declared PLAN SLICE is one atomic contract - its members join
        # one component even when their files are disjoint (live run
        # DATACMP-3-0b48b5b6: fixture pair + governing test share no file
        # but cannot be parallelized apart).
        sid = t.get("slice")
        if sid:
            if sid in slice_owner:
                uf.union(t["id"], slice_owner[sid])
            else:
                slice_owner[sid] = t["id"]
        f = t["file"]
        if not f:
...
def _self_test():
    ...
    # V (live run DATACMP-3-0b48b5b6): a PLAN slice is an atomic contract -
    # its steps share no file, yet they must never be split across parallel
    # workers, and two INDEPENDENT slices still parallelize.
    _pv = {"steps": [
        {"file": "sample_data/src.xml", "action": "modify", "what": "w",
         "slice": "S2"},
        {"file": "sample_data/tgt.xml", "action": "modify", "what": "w",
         "slice": "S2"},
        {"file": "src/other.py", "action": "modify", "what": "w"},
    ]}
    _tv = tasks_from_plan(_pv)
    _sv = partition_by_files(_tv)
    ok("V: plan-slice members stay in ONE parallel slice even with "
       "disjoint files",
       any(sorted(x["id"] for x in s) == ["task-01", "task-02"]
           for s in _sv))
    ok("V: an unrelated step still parallelizes as its own slice",
       len(_sv) == 2
       and any([x["id"] for x in s] == ["task-03"] for s in _sv))
    ok("V: tasks carry their plan-slice id through the partition",
       all(x.get("slice") == "S2" for s in _sv for x in s
           if x["id"] in ("task-01", "task-02")))
    ...
    # planning.verify_plan refuses a plan whose steps deadlock on a
    # forward dependency, and accepts the same plan once the atomic
    # contract is declared as one slice. That protection is only real if
    # the slice SURVIVES the partition - a declared slice split across
    # two parallel workers reintroduces the exact deadlock the plan gate
    # just refused, one layer down and with no gate left to catch it.
    _t20_plan = {"steps": [
        {"file": "src/reader.py", "action": "modify", "what": "the fix"},
        {"file": "sample_data/source.xml", "action": "modify",
         "slice": "S2", "what": "five records"},
        {"file": "sample_data/target.xml", "action": "modify",
         "slice": "S2", "what": "paired five-record target"},
        {"file": "tests/test_end_to_end.py", "action": "modify",
         "slice": "S2", "what": "row-count assertions"}]}
    _t20_slices = partition_by_files(tasks_from_plan(_t20_plan))
    _t20_group = next((s for s in _t20_slices
                       if any(t["id"] == "task-02" for t in s)), [])
    ok("T20-E3-b: a declared plan slice survives the partition as ONE "
       "sequential unit - three file-DISJOINT members that only make "
       "sense together are never handed to three parallel workers",
       sorted(t["id"] for t in _t20_group)
       == ["task-02", "task-03", "task-04"])
    ok("T20-E3-b: ...and a genuinely independent step still parallelizes "
       "- cohesion is not an excuse to serialize everything",
       len(_t20_slices) == 2
       and any([t["id"] for t in s] == ["task-01"] for s in _t20_slices))
    ok("T20-E3-b: ...and the file-disjointness safety net still holds "
       "over the merged result, so cohesion never buys a collision",
       verify_disjoint(_t20_slices) == []
       and all(t.get("slice") == "S2" for t in _t20_group))
