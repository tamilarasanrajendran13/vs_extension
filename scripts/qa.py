...
def generate_mock_data(manifest, project_path, cfg=None):
    ...
    for ds in (manifest.get("datasets") or []):
        ...
        # The manifest is MODEL-AUTHORED: an absolute path or a ../ escape
        # would write outside the project (Path(project) / "/etc/x" IS
        # /etc/x). Refuse anything that does not resolve inside the project.
        dest = pp / rel
        try:
...
def run_qa(tx, cfg, run_id, ticket_id, ticket_text, spec, patterns, radius,
    ...
    try:
        import run_context
        run_ctx = run_context.render_for(dev, "qa")
    except Exception:
        run_ctx = ""
    manifest, manifest_err = None, None
    for attempt in (1, 2):
        user = _qa_prompt(ticket_id, ticket_text, spec, patterns, frozen,
                          frozen_text=_frozen_contents(acc))
        if run_ctx:
            user += "\n\n" + run_ctx
        if manifest_err:
            user += ("\n\n=== YOUR PREVIOUS REPLY WAS NOT VALID JSON ===\n{}\n"
                     "Reply with exactly ONE JSON object.".format(
                         str(manifest_err)[:300]))
        try:
    ...
    for r in gen.get("refused") or []:
        say("  REFUSED fixture path outside the project: {}".format(r))
...
def _self_test():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # Model-authored fixture paths cannot escape the project.
        esc = generate_mock_data(
            {"datasets": [
                ...
                {"name": "good", "path": "test/fixtures/ok.csv", "rows": 2,
                 "columns": [{"name": "a", "type": "int"}]}]},
            str(td / "projr3"))
        ok("escaping fixture paths refused, good one written",
