...
def run_testspec(tx, cfg, run_id, ticket_id, ticket_text, spec, patterns,
    ...
    for bi, batch in enumerate(batches, 1):
        ...
        # One malformed reply must not silently cost the run 4 criteria (a real
        # e2e run failed its frozen_tests gate exactly this way: batch 2's reply
        # broke, its ACs stayed uncovered, the gate stopped the run). A batch is
        # a single cheap chat call - retry it with the parse error fed back
        # before declaring its criteria uncovered.
        ...
        if part is None:
            parse_failures.append("batch {}: {}".format(bi, last_err))
            continue
