...
def freeze_failure(problems, cov, budget_exhausted=False,
    ...
    # Windows demo mission (goal 4): when the criteria are uncovered
    # BECAUSE the focused coverage retry was refused by the stage's own
    # budget, saying "requirement ambiguity" blames the ticket for a
    # policy stop. It is the same budget_pause taxonomy member as above
    # - owner "policy", not retryable - and the UI renders it as a
    # budget stop, never an unexplained missing-AC defect.
    if missing and coverage_retry_refused:
        return ("budget_pause",
                "acceptance criteria {} are uncovered because the "
                "focused coverage retry was REFUSED by the stage's "
                "budget - a BUDGET stop, not a requirements defect and "
                "not a defective suite. Remedy: re-run at a deeper risk "
                "profile (Run with Overrides -> Risk Profile: Medium, "
                "or --risk-profile medium).".format(", ".join(missing)))
...
def run_testspec(tx, cfg, run_id, ticket_id, ticket_text, spec, patterns,
    ...
    # Windows demo mission (goal 4): whether the coverage retry that
    # would have addressed the missing criteria was REFUSED by budget -
    # freeze_failure types that as budget_pause, never as a
    # requirements defect.
    _cov_retry_refused = False
    if cov["missing"] and not _fast_reserve(_fstated,
        ...
        try:
            ...
            _cov_retry_refused = True   # goal 4: a budget refusal, typed
        except _TYPED_STOPS:
            _fsettle_once(None)
    ...
    if outcome == "fail":
        ...
        out["failure_class"], out["failure_evidence"] = \
            freeze_failure(problems, cov,
                           ...
                           coverage_retry_refused=_cov_retry_refused)
...
def _self_test():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # ---- WINDOWS DEMO MISSION (goal 4): the exact observed live
        # failure as a deterministic regression contract. First reply is
        # invalid JSON ("Extra data"); the correction parses but covers
        # only AC1+AC3; AC2 stays missing; under LOW the focused
        # coverage retry is refused by the TOTAL output budget and the
        # gate must stop as a TYPED budget_pause (a budget/policy stop,
        # never an unexplained AC2 defect); under MEDIUM the same two
        # replies plus the focused retry produce the complete 3/3
        # frozen suite. Nothing is fabricated on either path and the
        # ledger reports every call and refusal.
        _dm_spec = {"intent": "x", "acceptance_criteria": [
            {"text": "c1", "testable": True},
            {"text": "c2", "testable": True},
            {"text": "c3", "testable": True}]}
        _dm_bad = ('{"framework": "pytest", "validation_plan": "cut"}\n'
                   '{"tests": []}')
        try:
            parse_json(_dm_bad)
            _dm_bad_raises = False
        except Exception as _dm_e:
            _dm_bad_raises = "Extra data" in str(_dm_e)
        ok("[demo/AC2] the fixture's first reply really is the observed "
           "parse failure shape (Extra data)", _dm_bad_raises)
        _dm_partial = json.dumps({
            "framework": "pytest", "validation_plan": "observed",
            "tests": [_t("T1", "AC1", "dm1"), _t("T3", "AC3", "dm3")],
            "uncovered": []})
        _dm_focus = json.dumps({
            "framework": "pytest", "validation_plan": "focus",
            "tests": [_t("T2", "AC2", "dm2")], "uncovered": []})

        led = _FakeLedger(); ledger = led
        _dm_say = []
        _dm_tx = _FakeTransport([_dm_bad, _dm_partial],
                                tokens_out=[3002, 751])
        _dm_low = run_testspec(
            _dm_tx, {"_risk_profile": {"level": "low"}},
            "DM-LOW-run", "DM-LOW", "t", _dm_spec, None, [], "onetest",
            None, str(_mk_wb(Path(td) / "wbdmlow")), None, "ledger.db",
            _dm_say.append)
        ok("[demo/AC2] LOW: exactly two model calls - the malformed "
           "reply plus its one correction; the focused retry is refused "
           "BEFORE any send", len(_dm_tx.calls) == 2)
        ok("[demo/AC2] LOW: the channel narrates the observed sequence "
           "(unparseable -> remainder retry -> retry refused)",
           any("unparseable" in s for s in _dm_say)
           and any("stage remainder" in s for s in _dm_say)
           and any("cannot fit the focused coverage retry" in s
                   for s in _dm_say))
        ok("[demo/AC2] LOW: coverage is honestly 2/3 with AC2 missing - "
           "no criterion is fabricated or silently marked covered",
           _dm_low["coverage"]["missing"] == ["AC2"]
           and len(_dm_low["tests"]) == 2
           and not any("AC2" in (t.get("acceptance_criteria") or [])
                       for t in _dm_low["tests"]))
        ok("[demo/AC2] LOW: the gate stops as a TYPED budget_pause - a "
           "budget/policy stop, never an unexplained AC2 defect",
           _dm_low["outcome"] == "fail"
           and _dm_low.get("failure_class") == "budget_pause")
        ok("[demo/AC2] LOW: the evidence names the refused retry, the "
           "standing criterion AND the operable remedy",
           "AC2" in str(_dm_low.get("failure_evidence"))
           and "deeper risk profile" in str(
               _dm_low.get("failure_evidence")))
        ok("[demo/AC2] LOW: the ledger reports the refusal accurately "
           "(the budget-exhausted row names the focused coverage retry)",
           any(l["payload"].get("text") == "fast-profile total output "
                                           "budget exhausted"
               and l["payload"].get("refused_call")
               == "focused coverage retry"
               for l in led.logs))
        ok("[demo/AC2] LOW: the gate row is a FAIL carrying the honest "
           "call count (2 sent, none invented)",
           any(g["name"] == "frozen_tests" and g["outcome"] == "fail"
               and g["details"].get("model_calls") == 2
               for g in led.gates))

        led = _FakeLedger(); ledger = led
        _dm_say2 = []
        _dm_tx2 = _FakeTransport([_dm_bad, _dm_partial, _dm_focus],
                                 tokens_out=[3002, 751, 900])
        _dm_med = run_testspec(
            _dm_tx2, {"_risk_profile": {"level": "medium"}},
            "DM-MED-run", "DM-MED", "t", _dm_spec, None, [], "onetest",
            None, str(_mk_wb(Path(td) / "wbdmmed")), None, "ledger.db",
            _dm_say2.append)
        ok("[demo/AC2] MEDIUM: the same malformed reply costs one "
           "correction, then the focused AC2 retry IS allowed - three "
           "calls total", len(_dm_tx2.calls) == 3)
        ok("[demo/AC2] MEDIUM: the focused retry names exactly AC2",
           "AC2" in _dm_tx2.calls[2]["user"])
        ok("[demo/AC2] MEDIUM: the complete 3/3 suite freezes - "
           "outcome pass, nothing uncovered, three frozen files",
           _dm_med["outcome"] == "pass"
           and _dm_med["coverage"]["missing"] == []
           and _dm_med["coverage"]["ratio"] == 1.0
           and len(_dm_med["frozen"]) == 3)
        ok("[demo/AC2] MEDIUM: no gate was disabled and no criterion "
           "changed - the pass is earned through coverage, threshold "
           "untouched",
           any(g["name"] == "frozen_tests" and g["outcome"] == "pass"
               and g["details"].get("coverage", {}).get("ratio") == 1.0
               for g in led.gates))
