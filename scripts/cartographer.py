...
def explore(tx, tools: dict, index: str, tree_hash: str, workbench: Path,
    ...
    for step in range(1, max_steps + 1):
        ...
        try:
            ...
            # (silent malformed turns read as a mysterious hang, then
            # 'budget exhausted').
            say(f"    [{step}] reply was not valid JSON ({str(e)[:80]}) - one look burned")
        ...
        if action == "list":
            ...
            if chars_read > max_chars:
                # ENFORCED, not advisory: an ignored budget note used to let
                # the transcript grow until the provider rejected the prompt.
                result = ("=== READ BUDGET EXHAUSTED ===\nNo more reads. Answer "
                          "from what you have, and put anything you could not "
                          "determine in 'unclear'.")
                say(f"    [{step}] read refused - budget exhausted")
            else:
                result = tools["read"](paths)
                chars_read += len(result)
                if len(result) > 20_000:
                    result = (result[:20_000] +
                              "\n=== TRUNCATED: 20000 of {} chars shown. Ask for "
                              "fewer files or a narrower slice. ===".format(len(result)))
                say(f"    [{step}] read {len(paths)} file(s)   {thought[:60]}")
                if chars_read > max_chars:
                    result += ("\n\n=== READ BUDGET EXHAUSTED ===\nNo more reads. "
                               "Answer from what you have, and put anything you "
                               "could not determine in 'unclear'.")
        else:
    ...
    # Budget spent. Ask for the answer rather than losing the work - and RETRY
    # a bad final reply once with the error fed back: everything learned in
    # the whole exploration hangs on this one parse.
    ...
    for fattempt in (1, 2):
        ...
        reply = tx.chat(A["model"], A["prompt"], transcript + tail)
        try:
            patterns = (parse(reply["text"]).get("patterns")) or {}
            break
        except ValueError as e:
            perr = e
            say(f"    final reply attempt {fattempt} unparseable ({str(e)[:60]})"
                + (" - retrying" if fattempt < 2 else ""))
    if not patterns:
        ...
        return {"architecture": "", "extension_points": [], "conventions": [],
                ...
                "tree_hash": "", "budget_exhausted": True, "steps": steps,
                "steps_used": max_steps, "chars_read": chars_read,
