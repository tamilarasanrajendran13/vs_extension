...
def _make_real_worker(tx, run_id, ticket_id, ticket_text, spec, patterns, radius,
    ...
    def worker(worker_id, slice_tasks, cfg, coaching):
        ...
        outcome = "pass" if result.get("outcome") == "pass" else "fail"
        if outcome != "pass":
            ...
            try:
                checkpointer.Checkpointer.open(
...
def _self_test():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        res3b = run_lead_developer(_FakeTx(coach=recoach), cfg, "R3b", "OT-3b",
                                   ...
                                   run_unit=green_unit)
        ok("plan dispute skips coaching - each slice runs exactly once",
           rounds_run["n"] == len(led.gates[-1]["details"]["workers"]))
        ok("plan dispute reported with the developer's reason",
           res3b["failed"] and
           all("plan disputed" in (f["report"] or "") for f in res3b["failed"]))
