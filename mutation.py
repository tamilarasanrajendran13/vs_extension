...
def run_mutation(project_path, touched_py, cfg, run=None, cap=DEFAULT_CAP):
    ...
    return {"total": total, "killed": killed, "survived": survived,
            "kill_rate": kill_rate, "survivors": survivors, "capped": total >= cap}
...
def run_mutation_stage(tx, cfg, run_id, ticket_id, ticket_text, spec, patterns,
    ...
    say("mutating {} file(s), cap {} mutants...".format(len(touched_py), cap))
    ...
    ledger.gate(run_id, ticket_id, "mutation", outcome, score=result["kill_rate"],
...
def _self_test():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # A runner scripted so the first mutant is killed (fail) and the second
        # survives (pass); the rest killed.
        seq = iter([1, 0] + [1] * 50)  # 1 = returncode (fail=killed), 0 = pass=survived
        ...
        # all mutants killed -> pass
        _run = lambda cmd, cwd: type("P", (), {"stdout": "", "returncode": 1})()
