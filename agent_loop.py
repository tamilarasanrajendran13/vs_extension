...
def _trim(turns: list) -> None:
    ...
    the transcript budget. The two most recent turns are never collapsed."""
    total = sum(len(t) for t in turns)
    i = 0
    while total > MAX_TRANSCRIPT_CHARS and i < len(turns) - 2:
        t = turns[i]
        pos = t.find("\n=== RESULT:\n")
...
def _self_test() -> int:
    ...
    ok.append(("trim never touches the two most recent turns",
               _COLLAPSED not in turns[2] and _COLLAPSED not in turns[3]))
