...
OVERLAY_FILES = (
    ...
    # Carries the source-level RF2 FREEZE guard (no docket.showRunFlow2
    # in the command surface, no RF2 module in extension/src, no RF2
    # reference in shipped source) plus the 29-command contract. It
    # must ship, or the kit's own ladder could not enforce the freeze -
    # and an unlisted edit here would silently stay at the ref.
    "docket/extension/scripts/level2_suite.js",
    "docket/run_all_checks.py",
    "docket/tools/build_distribution.py",
    "docket/extension/.vscodeignore",
    # Windows demo mission (0.0.3, V4.4-BASED LEAN BACK-PORT): exactly
    # the Windows demo fixes on top of the v44 ref. Run Flow 2.0 and
    # the Live Workflow Queue are DELIBERATELY ABSENT - frozen until
    # after the demo by Tamil's decision. The self-test pins this list;
    # a package missing one of these would ship 0.0.1 behavior under a
    # 0.0.3 name, and a package CONTAINING an RF2 file would break the
    # freeze.
    "docket/loop.py",
    "docket/project_preflight.py",
    "docket/scripts/developer.py",
    "docket/scripts/test_spec.py",
    "docket/extension/package.json",
    "docket/extension/src/convenience.js",
    "docket/extension/src/probe.js",
    "docket/tools/windows_readiness.ps1",
    "docket/tools/windows_readiness_check.py",
    # 0.0.4 - the no-Jira override-loss fix. gateway.js forwards the
    # overrides picker's flags through the "Run From File" fallback and
    # runLocal splices them onto the spawn; extension.js accepts them on
    # the command. e2e_nine_stage.js and test/host/suite.js carry the
    # regression proof into the shipped kit's own ladder.
    "docket/extension/extension.js",
    "docket/extension/src/gateway.js",
    "docket/extension/scripts/e2e_nine_stage.js",
    "docket/extension/test/host/suite.js",
)
...
# Filename fragments no packaged file may carry, and source tokens no
# packaged file may contain. Checked on the STAGED tree and again on
# the EXTRACTED package, so an RF2 artifact cannot arrive from the ref,
# from an overlay, or from a hand-edited staging directory.
RF2_FILE_FRAGMENTS = ("run_flow_2", "workflow_queue_projection",
                      "tower_v6", "live_workflow_queue")
RF2_SOURCE_TOKENS = ("showRunFlow2", "docketRunFlow2",
                     "workflow_queue_projection", "run_flow_2")
# Files that legitimately NAME the freeze without shipping it. A guard
# has to spell out what it forbids, so the two files whose JOB is to
# refuse RF2 are exempted from the token scan by EXACT PATH - never by
# directory or pattern, so nothing else can hide behind them. The
# filename scan (RF2_FILE_FRAGMENTS) still applies to them, and
# level2_suite.js additionally asserts RF2 absence from inside the
# shipped kit's own ladder, so the exemption creates no blind spot.
# Found by the guard itself: the first lean build REFUSED to package
# because level2_suite.js names showRunFlow2 in its freeze checks.
RF2_SCAN_EXEMPT = ("docket/tools/build_distribution.py",
                   "docket/extension/scripts/level2_suite.js")


def verify_rf2_absent(root: Path) -> list[str]:
    """The freeze guard: return every violation proving Run Flow 2.0 /
    Live Workflow Queue material reached this tree (empty = frozen out).

    Three ways it can leak, all covered: a FILE whose name carries an
    RF2 fragment; a package.json contributing an RF2 COMMAND; and an
    RF2 REGISTRATION token inside any shipped .js/.py/.json source."""
    bad = []
    for p in sorted(Path(root).rglob("*")):
        ...
        low = p.name.lower()
        if any(f in low for f in RF2_FILE_FRAGMENTS):
            bad.append("{} [rf2-file]".format(rel))
            continue
        if rel in RF2_SCAN_EXEMPT:
            continue
        if p.suffix.lower() not in (".js", ".py", ".json"):
            continue
        try:
            text = p.read_text(encoding="utf-8", errors="replace")
        except OSError:
            continue
        for tok in RF2_SOURCE_TOKENS:
            if tok in text:
                bad.append("{} [rf2-token:{}]".format(rel, tok))
                break
    return bad


def verify_overlay_parity(kit_root: Path, repo: Path) -> list[str]:
    """The stale-package guard (Windows demo mission, goal F): every
    OVERLAY_FILES entry that exists in the working tree must be
    byte-identical inside the staged/extracted kit. Returns the
    drifted paths (empty = parity holds). Deterministic and pure."""
    bad = []
    for rel in OVERLAY_FILES:
        src = Path(repo) / rel
        if not src.is_file():
            continue
        dst = Path(kit_root) / rel
        try:
...
def _stage_clean_base(repo: Path, ref: str, workdir: Path,
    ...
    # Windows demo mission (goal F): the staged overlay bytes must equal
    # the working tree - a failed/partial copy must never mint a package
    # that silently omits a fix. config.json is sanitized AFTER overlays
    # on purpose and is not an overlay, so parity is checked here, last.
    drift = verify_overlay_parity(kit, repo)
    if drift:
        raise BuildError("overlay-parity",
                         "staged overlay differs from the working "
                         "tree: " + ", ".join(drift[:6]))
    # THE FREEZE, enforced in the pipeline (not only in the self-test):
    # RF2 / Live Workflow Queue material may not be packaged.
    rf2 = verify_rf2_absent(kit)
    if rf2:
        raise BuildError("rf2-freeze-violation",
                         "Run Flow 2.0 / Live Workflow Queue material "
                         "reached the staged kit: " + ", ".join(rf2[:6]))
...
def _verify_package(kit_zip: Path, workdir: Path, identity: Identity,
    ...
    # The freeze re-checked on the EXTRACTED bytes - the shipped
    # artifact is what a user opens, so it is what must be proven.
    rf2 = verify_rf2_absent(kit)
    if rf2:
        raise BuildError("rf2-freeze-violation",
                         "extracted package carries Run Flow 2.0 / "
                         "Live Workflow Queue material: "
                         + ", ".join(rf2[:6]))
...
def _self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # ride into the package; ledger.py is NOT listed and must stay
        # ref-exact (the dirty tree can never leak in wholesale).
        # (loop.py graduated INTO the overlay set with the Windows demo
        # fixes, so the unlisted fixture moved to ledger.py.)
        (repo / "docket" / "README.md").write_text(
            "corrected readme: install the VSIX\n")
        (repo / "docket" / "ledger.py").write_text(
        ...
        ok("overlay: an UNLISTED dirty worktree file stays ref-exact",
           (kit / "docket" / "ledger.py").read_text()
           == "# marker\n")
        # Windows demo mission (0.0.3 LEAN V4.4 BACK-PORT): every fix
        # file of the mission MUST be in OVERLAY_FILES - a stale
        # tag/overlay package that omits one would ship the broken
        # 0.0.1 behavior under a 0.0.3 name. Pinned by literal list.
        _fix_files = (
            "docket/containment.py",
            "docket/loop.py",
            "docket/project_preflight.py",
            ...
            "docket/tools/windows_readiness.ps1",
            "docket/tools/windows_readiness_check.py",
            # 0.0.4, the no-Jira override-loss fix. These two are RUNTIME:
            # gateway.js forwards the overrides picker's flags through the
            # "Run From File" fallback, extension.js accepts them on the
            # command. Left off this list the package would ship the
            # 0.0.3 behaviour - silently discarding every gate, budget,
            # model and risk override for local-file tickets - under a
            # 0.0.4 name, which is exactly the stale-overlay failure this
            # pin exists to catch.
            "docket/extension/extension.js",
            "docket/extension/src/gateway.js",
            # ...and the two suites that prove it, so the kit's own ladder
            # carries the regression rather than only this working tree.
            "docket/extension/scripts/e2e_nine_stage.js",
            "docket/extension/test/host/suite.js",
        )
        ok("overlay: every Windows-demo fix file rides OVERLAY_FILES "
           "(missing: {})".format(
               [f for f in _fix_files if f not in OVERLAY_FILES] or "none"),
           all(f in OVERLAY_FILES for f in _fix_files))
        # THE FREEZE (Tamil's decision, demo-first): Run Flow 2.0 and
        # the Live Workflow Queue are frozen until after the Windows
        # demo. No RF2 file may be listed for overlay, and no RF2 file,
        # command or registration may appear in a built package. This
        # is asserted on the EXTRACTED kit below as well - a list check
        # alone could not catch an RF2 file arriving from the ref.
        ok("freeze: no Run Flow 2.0 / Live Workflow Queue path is in "
           "OVERLAY_FILES", not [f for f in OVERLAY_FILES
                                 if "run_flow_2" in f
                                 or "workflow_queue" in f])
        _frz = globals().get("verify_rf2_absent")
        ok("freeze: verify_rf2_absent exists and passes on the "
           "extracted kit", _frz is not None and _frz(kit) == [])
        if _frz is not None:
            _plant = kit / "docket" / "extension" / "src" / "run_flow_2.js"
            _plant.write_text("// planted RF2 file\n")
            ok("freeze: a planted RF2 FILE in the kit IS caught",
               any("run_flow_2.js" in v for v in _frz(kit)))
            _plant.unlink()
            _pkg_p = kit / "docket" / "extension" / "package.json"
            _pkg_orig = _pkg_p.read_bytes()
            _pkg_j = json.loads(_pkg_orig)
            _pkg_j.setdefault("contributes", {}).setdefault(
                "commands", []).append(
                {"command": "docket.showRunFlow2", "title": "RF2"})
            _pkg_p.write_text(json.dumps(_pkg_j, indent=2))
            ok("freeze: a planted docket.showRunFlow2 COMMAND is caught",
               any("showRunFlow2" in v for v in _frz(kit)))
            _pkg_p.write_bytes(_pkg_orig)
            _reg_p = kit / "docket" / "extension" / "src" / "a.js"
            _reg_orig = _reg_p.read_bytes()
            _reg_p.write_bytes(_reg_orig + b'\n// docket.showRunFlow2\n')
            ok("freeze: a planted RF2 REGISTRATION inside a shipped "
               "source file is caught",
               any("a.js" in v and "rf2-token" in v for v in _frz(kit)))
            _reg_p.write_bytes(_reg_orig)
        # The stale-package guard itself: packaged overlay bytes must
        # equal the working tree, and a drifted byte is DETECTED.
        _vp = globals().get("verify_overlay_parity")
        ok("parity: verify_overlay_parity exists and passes on the "
           "extracted kit", _vp is not None and _vp(kit, repo) == [])
        if _vp is not None:
            _pf = kit / "docket" / "README.md"
            _orig = _pf.read_bytes()
            _pf.write_bytes(b"stale bytes that must be caught\n")
            ok("parity: a packaged overlay that drifts from the "
               "working tree IS caught",
               _vp(kit, repo) == ["docket/README.md"])
            _pf.write_bytes(_orig)
        ...
        # Windows demo mission (goals B + section 11): onboarding must
        # tell the user WHEN to run the project-runtime preflight, and
        # the optional readiness checker must be labeled diagnostic-only.
        ok("START-HERE: the probe re-run after Select Project is an "
           "explicit onboarding step (PROJECT-RUNTIME)",
           "PROJECT-RUNTIME" in sh_text
           and sh_text.index("Select Project")
           < sh_text.index("PROJECT-RUNTIME"))
        ok("START-HERE: the optional Windows readiness checker is "
           "documented as diagnostic only - installs nothing, changes "
           "nothing, deletable",
           "windows_readiness.ps1" in sh_text
           and "diagnostic only" in sh_text
           and "installs nothing" in sh_text
           and "changes nothing" in sh_text
           and "deleted after validation" in sh_text)
        ...
        # THE VERSION COUPLING. Artifact names come from
        # extension/package.json, but windows_readiness.ps1 pins the
        # version it EXPECTS to find in the kit. If those two ever
        # drift, the VM installs a kit whose own checker rejects it
        # (KIT-VERSION FAIL) - a self-inflicted blocker discovered only
        # on the demo machine. They are compared here instead.
        _repo = Path(__file__).resolve().parent.parent
        _pkg_v = ""
        _ps1_v = ""
        try:
            _pkg_v = json.loads(
                (_repo / "extension" / "package.json")
                .read_text(encoding="utf-8"))["version"]
        except Exception:                                 # noqa: BLE001
            _pkg_v = ""
        try:
            _m = re.search(
                r'\$script:ExpectedKitVersion\s*=\s*"([^"]+)"',
                (_repo / "tools" / "windows_readiness.ps1")
                .read_text(encoding="utf-8"))
            _ps1_v = _m.group(1) if _m else ""
        except Exception:                                 # noqa: BLE001
            _ps1_v = ""
        ok("version coupling: extension/package.json ({}) and the "
           "readiness checker's ExpectedKitVersion ({}) agree"
           .format(_pkg_v or "?", _ps1_v or "?"),
           bool(_pkg_v) and _pkg_v == _ps1_v)
        ok("the 0.0.2 Windows-demo release is superseded, so the kit "
           "version has moved past it (is {})".format(_pkg_v or "?"),
           _pkg_v not in ("", "0.0.1", "0.0.2"))
        # The checker's own gate has to be discoverable, or the operator
        # runs an unvalidated checker - which is exactly how the 0.0.2
        # defect reached a demo machine.
        ok("START-HERE tells the operator to validate the readiness "
           "checker itself first (-SelfTest) and to pass -Root",
           "-SelfTest" in START_HERE and "-Root" in START_HERE)
