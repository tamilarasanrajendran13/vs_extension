...
def run_security(tx, cfg, run_id, ticket_id, ticket_text, spec, patterns,
    ...
    try:
        reply = tx.chat(A["model"], A["prompt"], _triage_prompt(findings, diff))
    except Exception as e:
        ...
        return {"outcome": "unknown", "reason": "triage model call failed",
                "findings": findings}
    ledger.log(run_id, ticket_id, AGENT_NAME, "message", {"text": "triaged findings"},
    ...
    try:
        ...
        triaged = []  # fail-closed: unparseable triage dismisses nothing
