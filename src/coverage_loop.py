...
def _self_test():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # Every check that is NOT about mutation feedback injects this no-op.
        # Without it the self-test leaks into the REAL mutation engine and real
        # pytest, and the verdict depends on what the HOST has installed: on a
        # machine where `import mutation` fails the leak is invisible (false
        # green), on one where it works the strengthen loop adds entries the
        # exact-count checks do not expect. A self-test must not depend on the
        # weather.
        def no_mut(repo_, files, cfg):
            return {"kill_rate": None, "survived": 0, "survivors": [],
                    "skipped": "not exercised in this check"}
        ...
        res = run(_FakeTx(), {}, str(repo), workbench=str(repo), say=lambda *_: None,
                  measure=m, scan_fn=scan(cov=66.0), mutate_fn=no_mut)
        ...
        rrr = run(_FakeTx(), {}, str(repo), workbench=str(repo), only=["src/m.py::keep"],
                  say=lambda *_: None, scan_fn=scan(), mutate_fn=no_mut,
        ...
        r3 = run(_FakeTx(), {}, str(repo), workbench=str(repo), only=["src/m.py::keep"],
                 say=lambda *_: None, mutate_fn=no_mut,
