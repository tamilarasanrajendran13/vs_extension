...
def run_planner(tx, cfg: dict, run_id: str, ticket_id: str, ticket_text: str,
    ...
    for i, role in enumerate(roles, 1):
        ...
        say(f"  planner {i} ({role})...")
        try:
            r = agent_loop.run(tx, agent, tools, user, A.get("max_steps", 8),
                               done_key="plan", say=say)
        except ValueError as e:
            say(f"    planner {i} did not return JSON: {e}")
            continue
        plan = r["result"]
        if not plan:
            say(f"    planner {i} produced nothing")
            continue
        ...
        if plan.get("radius_problem"):
            say(f"    planner {i} says the radius is wrong: {plan['radius_problem']}")
        ...
        if violations:
            say(f"    planner {i}: {len(violations)} violation(s)")
            for v in violations:
                say(f"      {v['file'] or '(plan)'}: {v['problem'][:90]}")
    ...
    if len(plans) == 1:
        ...
        try:
            ...
            label = j.get("winner")
            idx = planning.LABELS.find(label) if label else 0
            winner = plans[idx if 0 <= idx < len(plans) else 0]
            say(f"    winner: plan {label} ({mapping.get(label)}) - {j.get('why', '')[:100]}")
