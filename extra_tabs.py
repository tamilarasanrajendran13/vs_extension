...
def reference_section():
    rows = [
        ("Fetch + comprehend", "spec agent", "deterministic pre-gates; 3-state gate", False),
        ("Context + map", "drafter, cartographer", "ratified by you; paths verified", True),
        ("Declare scope", "lead", "hook blocks edits outside the radius", True),
        ("Plan", "planner x2-3 + blind judge", "judge picks; you may review", False),
        ("Freeze tests", "test-spec", "frozen; developer cannot edit them", False),
        ("Write code", "developer / lead-developer", "hook blocks out-of-radius; per-task checkpoints", False),
        ("Roll back", "YOU decide", "checkpointer proves byte-identical", True),
        ("Blind review", "reviewer", "sees diff + ticket only", False),
        ("Security", "scanner finds, agent triages", "fail-closed on high findings", False),
        ("QA", "qa / lead-qa", "frozen suite is authoritative", False),
        ("Mutation", "deterministic engine + triage", "kill-rate gate", False),
    ...
    cmds = [
        ("Run a ticket", "python loop.py --stdio", "Drive one ticket through the pipeline via the VS Code gateway."),
        ("Dashboard", "python report.py --db ledger.db --out report.html",
         "Build this self-contained report (these tabs included)."),
        ("Lead runs", 'config.json: "governor": { "parallel_dev": true, "parallel_qa": true }',
...
<div class="xt-own">
  ...
  <div class="xt-a"><h3>Runs without you</h3><ul>
    ...
    <li>Freezing tests, units, mutation, security, QA</li>
    <li>Blind peer review; coaching failing slices</li>
