...
<p class="xt-ey" style="margin-top:30px">The gates</p>
<h2>What each gate actually checks</h2>
<p class="xt-lede">Nine gates, in order. Each returns pass, fail, or unknown -
an unrun check is never a silent pass.</p>
<table><thead><tr><th>Gate</th><th>The question it answers</th></tr></thead><tbody>
<tr><td class="xt-stage">Comprehension</td><td>Is the ticket clear enough to build from? Acceptance criteria testable and unambiguous, before any work starts.</td></tr>
<tr><td class="xt-stage">Context</td><td>Has the blast radius been mapped and ratified? Which files and boundaries the change may touch.</td></tr>
<tr><td class="xt-stage">Plan</td><td>Is there a sound implementation plan? Often a blind bake-off judged against the frozen acceptance criteria.</td></tr>
<tr><td class="xt-stage">Test spec</td><td>Are the tests written before the code, and frozen? So the implementation cannot move the goalposts.</td></tr>
<tr><td class="xt-stage">Develop</td><td>The implementation itself - code written against the frozen plan and tests, inside the blast radius.</td></tr>
<tr><td class="xt-stage">Review</td><td>Does it hold up to blind peer review? The reviewer sees the diff and the ticket only.</td></tr>
<tr><td class="xt-stage">Security</td><td>Any vulnerabilities? Snyk and dependency/code analysis; fail-closed on high findings.</td></tr>
<tr><td class="xt-stage">QA</td><td>Does it behave end-to-end against the acceptance criteria, using the frozen suite?</td></tr>
<tr><td class="xt-stage">Mutation</td><td>Do the tests actually catch bugs? The code is deliberately broken to see if the tests notice - the kill-rate gate, not coverage.</td></tr>
</tbody></table>
<p class="xt-ey" style="margin-top:30px">On disk</p>
<h2>What gets written where</h2>
<p class="xt-lede">Each ticket gets a folder,
<code>development/&lt;release&gt;/&lt;ticket&gt;/</code>, so the whole run is browsable
after the fact - not just queryable in the ledger.</p>
<table><thead><tr><th>Folder</th><th>Written by</th><th>Holds</th></tr></thead><tbody>
<tr><td class="xt-stage">context/</td><td>spec, cartographer, drafter</td><td>ticket + comprehension, the repo map, the ratified context</td></tr>
<tr><td class="xt-stage">plan/</td><td>lead, planner, judge, test-spec</td><td>blast radius, candidate plans, the chosen plan, the frozen tests</td></tr>
<tr><td class="xt-stage">implementation/</td><td>developer, reviewer, security</td><td>the diff summary, the peer review, the security triage</td></tr>
<tr><td class="xt-stage">test/</td><td>qa, mutation</td><td>unit and end-to-end results, the mutation report</td></tr>
<tr><td class="xt-stage">evidence/</td><td>retro, report</td><td>the retrospective and the run's report</td></tr>
</tbody></table>
</section>"""
