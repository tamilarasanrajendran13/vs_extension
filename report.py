...
def build_report(db: str, out: str, release=None, project=None, max_events=200,
    ...
    # them automatically, so no app.js/app.css change is needed. Their data is
    # read from the same ledger and baked in here, so they refresh every build.
    # Never fatal: a report without the extra tabs beats no report.
    try:
        import extra_tabs
        html = extra_tabs.inject(html, db)
...
def _self_test() -> int:
    ...
    if _summary_col:
        con.execute('UPDATE runs SET "%s" = ? WHERE rowid = 1' % _summary_col,
