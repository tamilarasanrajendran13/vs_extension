...
KNOWN_OUTCOMES = ["merged", "halted", "failed", "running"]
...
def _build(con, release, project, event_limit, max_rows=MAX_ROWS_PER_TABLE,
    ...
    trend = _release_trend(all_runs, all_by_ticket, comp_halt_run_ids)
    ...
    def _run_key(r):
        # Must mirror _key: if gates/events carry a run id we key by the run's
        # own id; otherwise everything keys by ticket. Mixing the two (runs by
        # id, gates by ticket) empties every walk - which is exactly the bug the
        # 'timeline attached' and 'halt mark' tests catch.
        gates_have_run = any(g.get("run") is not None for g in gates)
        if gates_have_run and r.get("_id") is not None:
            return r["_id"]
        return r["issue"]
    ...
    # collapse runs into one row per ticket (latest run defines the row)
    tickets = _group_by_ticket(tickets)
    ...
    finished = sum(counts.get(o, 0) for o in ("merged", "failed", "halted"))
    ...
    totals = {
        ...
        **counts,
        ...
        "first_pass_rate": _div(counts.get("merged", 0), finished) if finished else None,
...
def _release_trend(all_runs: list[dict], evs_by_ticket: dict,
                   comp_halt_run_ids: set = None) -> list[dict]:
    """
    ...
    for rel in sorted(by_rel):
        ...
        decided = sum(counts.get(o, 0) for o in ("merged", "failed", "halted"))
        ...
        out.append({
            ...
            **counts,
            "cost_usd": total_cost,
            "cost_per_ticket": _div(total_cost, len(priced)),
            "first_pass_rate": _div(counts.get("merged", 0), decided) if decided else None,
            "halt_rate": _div(counts.get("halted", 0), decided) if decided else None,
            "fail_rate": _div(counts.get("failed", 0), decided) if decided else None,
...
def _group_by_ticket(runs: list[dict]) -> list[dict]:
    """
    One row per ticket, newest run on top; every run kept underneath.
    ...
    for r in runs:
        by_ticket.setdefault(r["issue"], []).append(r)

    out = []
    for issue, attempts in by_ticket.items():
        # newest first; the latest run defines the collapsed row
        attempts.sort(key=lambda r: str(r.get("started") or ""), reverse=True)
        latest = attempts[0]
    ...
    out.sort(key=lambda t: str(t.get("started") or ""), reverse=True)
    return out
