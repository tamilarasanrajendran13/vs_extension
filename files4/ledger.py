...
def start_run(ticket_id: str, project: str = "unknown",
    ...
    with connect(db) as con:
        con.execute(
            "INSERT INTO runs (run_id, ticket_id, project, origin, outcome, "
            "budget_usd, git_sha_start) VALUES (?,?,?,?,'running',?,?)",
            (run_id, ticket_id, project, origin(), budget_usd, git_sha),
        )
    return run_id
...
def _self_test() -> int:
    ...
    run_id = start_run("PROJECT-110", project="onetest", budget_usd=2.50, db=tmp)
...
CLI_COMMANDS = ("start-run", "log", "gate", "end-run", "write-dossier",
                "propose-learning", "resume", "transcript", "search", "danger-zones")
