...
def load_patterns(cfg: dict, tx, project: str, project_path: Path | None,
    ...
    """
    How this codebase is extended, read FROM the codebase.

    Two steps, deliberately split:
      map_repo.py    extracts the index. Facts. Deterministic. Free.
      cartographer   interprets it. Judgement. One model call.
    ...
    say(f"  repo changed - reading {m['stats']['modules']} modules "
        f"({len(index)} chars of index, not source)")
    try:
        ctx = load_project_context(workbench, project)
        p = cartographer.map_patterns(tx, index, th, workbench, project, ctx)
    except Exception as e:
        say(f"  cartographer failed: {e}")
        return ""

    eps = p.get("extension_points") or []
    say(f"  patterns: {len(eps)} extension point(s) found")
    for ep in eps:
        say(f"    - {ep.get('what')} via {ep.get('mechanism')} "
            f"[{ep.get('confidence')}]")
    if not eps:
        say(f"    none identified - the planner will have to look for itself")
    return cartographer.render(p)
