...
**"What testable means here"** is the second-most valuable section, and the one
you will not think to write.

Without it, an agent falls back on a generic heuristic - usually "testable means
it has a numeric threshold" - and rejects criteria like *"copybook parsing works
correctly"* because there is no number in it. On a data validation framework,
where every assertion is "source equals target", that heuristic rejects almost
everything and buries the ticket author in questions they cannot answer.

Testability is project-specific. What counts as testable for a latency-sensitive
API is not what counts for a validation framework. Only you know which.
