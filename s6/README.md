...
│   ├── loop.py              <- THE LOOP. Pure Python. No VS Code.
│   ├── transport.py         <- stdio | api | mock
│   ├── extension/           <- the model gateway. The ONLY VS Code dependency.
│   │   ├── package.json
│   │   ├── extension.js
│   │   └── src/             <- gateway, models, config, workspace, clone, probe
...
The loop is Python. VS Code is a **model provider**, not a host.

```
extension  --spawns-->  python loop.py --stdio
           <--JSON lines over the pipe-->
           vscode.lm  <--answers "chat" requests--
```

No socket, no port, no firewall prompt, nothing for endpoint protection to flag.
Same transport LSP and MCP use, for the same reasons.

**VS Code must be running** - `vscode.lm` exists only inside the extension host,
and your org has not enabled the Copilot CLI. Nothing in this design changes
that. What it does change: no chat window, no human, no session. Docket can poll
Jira at 2am with the window minimised.

Everything except the model call already runs standalone:

```bash
python loop.py --self-test     # 12/12, no VS Code, no network, no models
python ledger.py --self-test   # 21/21
```

The day Copilot CLI, a LiteLLM proxy, or API keys land, implement
`ApiTransport.chat()` and `python loop.py --api PROJ-110` runs from cron. The
loop above it does not change by one line - it never knew where the models came
from.

| | Lives in | Scope |
|---|---|---|
| The loop | `loop.py`, a child process | portable |
| `vscode.lm` calls | `extension/src/gateway.js` | the only VS Code tie |
| Python scripts | imported by the loop | the project |
...
- **`extension.js` stays thin.** Add a require and a registerCommand. Never grow
  it sideways.
- **The gateway stays dumb.** It answers `chat` and `models`. It must never learn
  what a ticket, an agent or a gate is. The moment it does, Docket is welded to
  VS Code again.
- **stdout is the wire.** In `loop.py`, anything human-readable goes to stderr or
  through `tx.progress()`. A stray `print()` corrupts the protocol.
