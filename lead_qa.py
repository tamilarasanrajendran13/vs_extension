...
def _shard(tx, A, frozen, say):
    ...
    user = ("QUESTION 1 (shard the suite)\n\nFrozen acceptance tests:\n"
            + "\n".join(frozen))
    try:
        reply = tx.chat(A["model"], A["prompt"], user, key="shard")
...
def _coach_shard(tx, A, shard, result, round_no, say):
    user = ("QUESTION 2 (coach a failing shard)\n\nShard {} tests: {}\nAttempt: {}\n\n"
            "FAILING OUTPUT:\n{}".format(shard["id"], ", ".join(shard["tests"]),
                                         round_no, result.get("failing", "")))
    try:
        move = parse_json(tx.chat(A["model"], A["prompt"], user, key="coach")["text"])
    except Exception:
...
def run_lead_qa(tx, cfg, run_id, ticket_id, ticket_text, spec, patterns, radius,
    ...
    shards = validate_shards(_shard(tx, A, frozen, say), frozen)
    if len(shards) <= 1:
    ...
    ledger.gate(run_id, ticket_id, "qa_e2e", outcome, actor=AGENT_NAME,
...
def _make_real_shard_worker(project_path, workbench, release, ticket_id, cfg):
    ...
    def shard_worker(worker_id, tests, wcfg, manifest):
        ...
        if res["total"] == 0:
            return {"outcome": "unknown", "detail": "no tests collected for shard"}
...
class _FakeTx:
    ...
    def chat(self, model, system, user, key=None):
        payload = ({"mode": "shard", "shards": self.shards} if key == "shard"
                   else self.coach)
