...
def run_reviewer(tx, cfg, run_id, ticket_id, ticket_text, spec, patterns,
    ...
    reply = tx.chat(A["model"], A["prompt"],
                    _blind_prompt(ticket_id, ticket_text, _acceptance(spec), diff))
    ledger.log(run_id, ticket_id, AGENT_NAME, "message", {"text": "reviewed the diff"},
    ...
    ledger.gate(run_id, ticket_id, "blind_review", outcome, actor=AGENT_NAME,
