...
ENV_ALLOW_EXACT = {
    ...
    "SHELL", "USER", "LOGNAME", "TERM", "JAVA_HOME", "SPARK_HOME",
    "SPARK_LOCAL_IP", "HADOOP_HOME", "HADOOP_CONF_DIR", "PYSPARK_PYTHON",
    "PYSPARK_DRIVER_PYTHON",
}
ENV_ALLOW_PREFIXES = ("LC_", "PYTHON", "PYTEST_", "COVERAGE_")

# THE Windows-runtime allowance (Windows demo mission). Windows
# environment names are case-insensitive, and Python's os.environ
# upper-cases them on Windows - so an exact-case entry ("SystemRoot")
# never matches the live "SYSTEMROOT". Stripping these broke the demo
# host twice: no SYSTEMROOT -> winsock provider load fails (OSError
# WinError 10106) the moment a child touches sockets/asyncio; no
# PROCESSOR_* -> polars' native CPU-feature detection dies
# (RuntimeError: unknown feature flag "sse3"). This is the MINIMUM
# runtime set, matched case-insensitively through
# is_windows_runtime_name() - the one authority; the secret pattern
# above still wins over it (PROCESSOR_AUTH_TOKEN dies).
ENV_ALLOW_WINDOWS_EXACT = frozenset({
    "SYSTEMROOT", "WINDIR", "SYSTEMDRIVE", "COMSPEC", "PATHEXT",
    "TEMP", "TMP", "NUMBER_OF_PROCESSORS",
    # IDENTITY PARITY with the POSIX allowance above. POSIX children
    # are given HOME/USER/LOGNAME; without these their Windows
    # equivalents were all stripped, leaving a contained Windows child
    # with NO home directory and NO user identity:
    # ntpath.expanduser("~") returned the literal "~", which breaks
    # every tool that writes under the user profile, and stops a Spark
    # session from starting (Java resolves user.home from USERPROFILE;
    # Hadoop's UserGroupInformation resolves the run user from
    # USERNAME) even though it starts fine directly - i.e. it
    # manufactures a direct-PASS/contained-FAIL split. Same defect
    # shape as SYSTEMROOT: POSIX name allowlisted, Windows equivalent
    # forgotten. The secret pattern still wins over all of these.
    "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "USERNAME",
    "APPDATA", "LOCALAPPDATA",
})
ENV_ALLOW_WINDOWS_PREFIXES = ("PROCESSOR_",)


def is_windows_runtime_name(name) -> bool:
    """True when NAME is a Windows runtime variable Python/native wheels
    require, matched case-insensitively (SystemRoot == SYSTEMROOT).
    Deliberately narrow: the allowance is these names and PROCESSOR_*,
    nothing more - secret denial is checked BEFORE this by
    sanitize_env, so a secret-shaped name never reaches it there."""
    u = str(name).upper()
    return (u in ENV_ALLOW_WINDOWS_EXACT
            or u.startswith(ENV_ALLOW_WINDOWS_PREFIXES))
...
def capabilities() -> dict:
    ...
    return {
        ...
        # Windows demo mission: the Windows branch of the kill seam.
        # Argv-verified on every host by the self-test; LIVE tree-kill
        # proof is only possible on a real Windows host (VM acceptance).
        "process_tree_kill_windows": "taskkill /F /T by PID "
                                     "(argv-verified here; live-proven "
...
def _kill_tree(p, platform=None, runner=None):
    """THE timeout kill, cross-platform (Windows demo mission).

    POSIX: the child got its own session (start_new_session=True), so
    killpg reaps the whole group - grandchildren (Spark JVMs) included;
    live-proven by the rc-124 + grandchild self-test checks.

    Windows: os.killpg/os.getpgid DO NOT EXIST - the old path died on an
    uncaught AttributeError instead of killing anything. taskkill /F /T
    kills the process TREE by PID (start_new_session is silently ignored
    by Popen on Windows, so there is no group to signal). p.kill() still
    fires as the direct fallback either way. The Windows branch is
    argv-verified in the self-test; live tree-kill proof belongs to the
    Windows VM acceptance run, and capabilities() says so."""
    plat = os.name if platform is None else platform
    if plat == "nt":
        run = subprocess.run if runner is None else runner
        try:
            run(["taskkill", "/F", "/T", "/PID", str(p.pid)],
...
def run_contained(cmd, cwd, timeout=900, env=None, allowed_roots=None,
    ...
    try:
        ...
        timed_out = True
        # FULL TREE kill through the one cross-platform seam (POSIX:
        # killpg on the child's own session; Windows: taskkill /F /T).
        _kill_tree(p)
        try:
...
def _self_test() -> int:
    ...
    with _tf.TemporaryDirectory() as td:
        ...
        # WINDOWS RUNTIME SURVIVAL (Windows demo mission): the variables
        # Python and native wheels REQUIRE on Windows must survive
        # sanitization, matched case-insensitively - Windows env names
        # are case-insensitive and Python upper-cases them in os.environ,
        # so an exact-case "SystemRoot" allowlist entry never matches the
        # real "SYSTEMROOT". Observed live on the demo host: stripping
        # SYSTEMROOT broke winsock (OSError WinError 10106); stripping
        # PROCESSOR_* broke polars CPU detection (RuntimeError: unknown
        # feature flag "sse3"). Simulated here on this host - the rule is
        # platform-independent by design so it IS testable here.
        _win_src = {
            ...
            "WINDOWS_API_KEY": "leak", "PROCESSOR_AUTH_TOKEN": "leak",
            "RANDOM_WIN_VAR": "noise",
            "PATH": "C:\\Windows;C:\\x", "TEMP": "C:\\t", "TMP": "C:\\t",
        }
        _win = sanitize_env(_win_src)
        check("windows runtime: mixed-case SystemRoot survives with its "
              "original name (case-insensitive match, name preserved)",
              _win.get("SystemRoot") == "C:\\Windows")
        check("windows runtime: SYSTEMROOT-shape names all survive "
              "(WINDIR, SYSTEMDRIVE, ComSpec, PATHEXT)",
              _win.get("WINDIR") == "C:\\Windows"
              and _win.get("SYSTEMDRIVE") == "C:"
              and "ComSpec" in _win and "PATHEXT" in _win)
        check("windows runtime: processor metadata survives "
              "(NUMBER_OF_PROCESSORS + every PROCESSOR_*)",
              _win.get("NUMBER_OF_PROCESSORS") == "8"
              and _win.get("PROCESSOR_ARCHITECTURE") == "AMD64"
              and "PROCESSOR_IDENTIFIER" in _win
              and "PROCESSOR_LEVEL" in _win
              and "PROCESSOR_REVISION" in _win
              and "PROCESSOR_ARCHITEW6432" in _win)
        check("windows runtime: secret deny STILL beats the windows "
              "allowance (WINDOWS_API_KEY, PROCESSOR_AUTH_TOKEN die)",
              "WINDOWS_API_KEY" not in _win
              and "PROCESSOR_AUTH_TOKEN" not in _win)
        check("windows runtime: unrelated names are still removed "
              "(the allowance is specific, not general)",
              "RANDOM_WIN_VAR" not in _win)

        # WINDOWS IDENTITY PARITY (audit of the 38-vs-40 baseline
        # discrepancy). POSIX children are given HOME/USER/LOGNAME;
        # their Windows equivalents were ALL stripped, so a contained
        # Windows child had no home directory and no user identity at
        # all. Proven consequence: ntpath.expanduser("~") returns "~"
        # unexpanded, which breaks every tool that writes under the
        # user profile - and is the most likely reason a Spark session
        # (Java user.home, Hadoop UserGroupInformation -> USERNAME)
        # cannot start contained while starting fine directly. That
        # shape - POSIX name allowlisted, Windows equivalent missing -
        # is exactly the SYSTEMROOT defect again.
        _id_src = {"USERPROFILE": "C:\\Users\\demo", "HOMEDRIVE": "C:",
                   "HOMEPATH": "\\Users\\demo", "USERNAME": "demo",
                   "APPDATA": "C:\\Users\\demo\\AppData\\Roaming",
                   "LOCALAPPDATA": "C:\\Users\\demo\\AppData\\Local",
                   "USERPROFILE_API_KEY": "leak"}
        _id = sanitize_env(_id_src)
        check("windows identity: the HOME equivalents survive "
              "(USERPROFILE, HOMEDRIVE, HOMEPATH) - POSIX gets HOME, "
              "Windows must not get nothing",
              _id.get("USERPROFILE") == "C:\\Users\\demo"
              and _id.get("HOMEDRIVE") == "C:"
              and _id.get("HOMEPATH") == "\\Users\\demo")
        check("windows identity: the USER equivalent survives "
              "(USERNAME) - Hadoop/Spark resolve the run user from it",
              _id.get("USERNAME") == "demo")
        check("windows identity: APPDATA/LOCALAPPDATA survive - native "
              "toolchains cache there when HOME does not exist",
              "APPDATA" in _id and "LOCALAPPDATA" in _id)
        check("windows identity: secret deny STILL wins over the "
              "identity allowance",
              "USERPROFILE_API_KEY" not in _id)
        import ntpath as _ntp
        _saved_env = dict(os.environ)
        try:
            os.environ.clear()
            os.environ.update(sanitize_env(_id_src))
            _expanded = _ntp.expanduser("~")
        finally:
            os.environ.clear()
            os.environ.update(_saved_env)
        check("windows identity: a CONTAINED child can expand '~' "
              "(ntpath.expanduser resolves to the profile, not the "
              "literal '~' the old sanitizer left it as)",
              _expanded == "C:\\Users\\demo")
        check("windows runtime: PATH/TEMP/TMP behavior unchanged",
              _win.get("PATH") == "C:\\Windows;C:\\x"
              and _win.get("TEMP") == "C:\\t" and _win.get("TMP") == "C:\\t")
        check("windows runtime: ONE testable authority exists "
              "(is_windows_runtime_name), no duplicate lists",
              globals().get("is_windows_runtime_name") is not None
              and is_windows_runtime_name("SystemRoot")
              and is_windows_runtime_name("PROCESSOR_ARCHITECTURE")
              and not is_windows_runtime_name("RANDOM_WIN_VAR")
              and not is_windows_runtime_name("PATH"))

        # WINDOWS TIMEOUT KILL (same mission): os.killpg/os.getpgid do
        # not exist on Windows - the old timeout path would die on an
        # UNCAUGHT AttributeError instead of killing the tree. The kill
        # now routes through one seam whose Windows branch composes
        # taskkill /F /T (kills the whole tree by PID) and still calls
        # p.kill() as the direct fallback. The POSIX branch is the
        # live-proven killpg path (rc-124 + grandchild checks above).
        # HONEST LIMITATION: the Windows branch is argv-verified here,
        # live-proven only on a real Windows host (the VM acceptance).
        _kt = globals().get("_kill_tree")
        _tk_calls = []
        ...
        if _kt is not None:
            _kt(_fp, platform="nt",
                runner=lambda argv, **kw: _tk_calls.append(list(argv)))
        check("windows kill-tree: the seam composes taskkill /F /T /PID "
              "and still fires the direct kill",
              _kt is not None
              and _tk_calls == [["taskkill", "/F", "/T", "/PID", "4242"]]
              and _fp.killed == 1)
        check("windows kill-tree: capabilities() states the Windows "
              "mechanism and its not-live-proven-here status honestly",
              "taskkill" in str(capabilities().get(
                  "process_tree_kill_windows", "")))
