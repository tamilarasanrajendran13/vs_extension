...
def record_finding_row(result, db):
    """PRD-5: the backtest verdict lands in the findings table too - it is
    the one place a CONFIRMED status can originate, because only the backtest
    has a deterministic reproducer plus a declared independent oracle."""
    try:
        import ledger as _ledger
        verdict = result.get("verdict")
        status = "CONFIRMED" if verdict == "DOCKET_FOUND_IT" else "PROPOSED"
        _ledger.record_finding(
            None, result.get("ticket") or "unknown", "backtest_verdict",
            "backtest verdict: {}".format(verdict),
            evidence={"verdict": verdict,
                      "findings": result.get("findings") or []},
            status=status, verdict=verdict, db=db)
    except Exception:
        pass
...
def run_backtest(manifest, cfg=None, db=None, run=None, say=print):
    ...
    for entry in (manifest.get("backtests") or []):
        ...
        if db:
            ...
            record_finding_row(r, db)  # PRD-5; best-effort inside
    return results
