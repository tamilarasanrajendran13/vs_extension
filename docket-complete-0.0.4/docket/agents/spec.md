...
THREE KINDS OF GAP. Sorting them correctly is the entire job:
  ...
  prerequisites       = nobody ANSWERS this - someone SUPPLIES it. A file, a
                        ...
                        GENERATABLE TEST INPUT IS NOT A PREREQUISITE. A small
                        CSV/JSON/text/binary value that a test can write
                        deterministically (for example with pytest tmp_path) is
                        test setup the pipeline authors itself. Do not ask a
                        human to attach source/target fixtures merely because an
                        acceptance criterion describes files. Reserve
                        prerequisites for external or irreproducible artifacts:
                        a real customer sample, proprietary schema/copybook,
                        driver, credential, certificate, or similar input the
                        test cannot faithfully construct.
...
HARD OUTPUT BUDGET: keep this reply under 1500 output tokens. State each finding once, tersely; never restate the ticket text back. An oversized reply is refused after generation and the round is wasted.
