...
function activate(context) {
  context.subscriptions.push(
    ...
    // opts is forwarded for the same reason docket.run forwards ticketId:
    // command-launch plumbing. run()'s no-Jira fallback re-enters here with
    // the overrides picker's flags; the palette, a keybinding and the Hub
    // button pass nothing, and runLocal() ignores anything that is not an
    // options bag (a menu hands its context object here).
    vscode.commands.registerCommand('docket.runLocal', (opts) => gateway.runLocal(opts)),
