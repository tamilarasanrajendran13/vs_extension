...
// config.json as it stood before the override run - an override must be
// argv only, so this must still match afterwards.
let cfgJsonBeforeRun = "";
...
async function runTheTicket() {
  ...
  // THE DEMO PATH, end to end. Not `docket.runLocal` directly any more:
  // "Run with Overrides" -> Risk Profile Medium -> this machine has no
  // Jira -> the refusal's "Run From File" escape hatch.
  //
  // That fallback is where the 0.0.4 override-loss defect lived:
  // gateway.run() re-entered docket.runLocal with NO arguments, so every
  // gate, budget, model and risk override was silently discarded for
  // local-file tickets - the run looked completely normal and simply
  // ignored what was asked for. It is also the exact route the demo
  // machine takes. Driving it HERE, rather than only against a mocked
  // helper, makes the flag survive a REAL `python3 -u loop.py --stdio`
  // child - the only place "requested/effective risk profile" can
  // actually be recorded.
  //
  // Jira is removed from the environment for the duration, so the refusal
  // is caused by the fixture and not by whatever the developer's shell
  // happens to export.
  cfgJsonBeforeRun = require("fs").readFileSync(
    path.join(WB, "config.json"), "utf8");
  const jiraSaved = {};
  for (const k of ["JIRA_BASE_URL", "JIRA_URL", "JIRA_PAT", "JIRA_TOKEN",
                   "JIRA_API_TOKEN"]) {
    if (k in process.env) { jiraSaved[k] = process.env[k]; delete process.env[k]; }
  }
  ok("the fixture really has no Jira, so the Run From File fallback below "
     + "is reached for a real reason - not by stubbing the check",
     !gateway.hasJiraCredentials(WB), WB);
  scriptQuickPick = (items, index, options) => {
    ...
    if (/step 3 of 4/.test(ph)) return items[0];        // keep configured model
    if (/step 4 of 4/.test(ph)) {
      return items.find((i) => /medium/i.test(i.label));
    }
    return items.find((i) => i.label === TICKET);       // the ticket picker
  };
  scriptAnswer = (kind, message) =>
    (/No Jira credentials/.test(message) ? "Run From File" : undefined);
  observing = true;
  await rec.commands.get("docket.runWithOverrides")();
  await settle(20);
  observing = false;
...
async function sectionSpawn() {
  ...
  // THE OVERRIDE-LOSS REGRESSION (0.0.4), proven against the REAL child.
  // The run above came in through Run with Overrides -> Medium -> no Jira
  // -> "Run From File". Before the fix the fallback re-entered
  // docket.runLocal with no arguments and the override never reached
  // python at all: same ticket, same nine stages, silently automatic risk.
  const ridx = s.args.indexOf("--risk-profile");
  ok("the Medium override SURVIVED the no-Jira Run From File fallback and "
     + "reached python's command line",
     ridx !== -1 && s.args[ridx + 1] === "medium", JSON.stringify(s.args));
  ok("...on the same command line as the LOCAL ticket file, and with no "
     + "--fetch - it is still a no-Jira run",
     s.args.includes("--ticket-file")
     && s.args[s.args.indexOf("--ticket-file") + 1]
        === path.join("tickets", TICKET + ".md")
     && !s.args.includes("--fetch"), JSON.stringify(s.args));
  // What the operator actually sees, from loop.py itself - not from the
  // extension repeating back what it sent.
  const chan = rec.channelLines.join("\n");
  ok("...and loop.py RECORDS it: the run says 'requested risk profile: "
     + "medium' and 'effective risk profile: medium'",
     /requested risk profile:\s*medium/.test(chan)
     && /effective risk profile:\s*medium/.test(chan),
     JSON.stringify(rec.channelLines.filter((l) => /risk profile/.test(l))));
  // And the ledger holds it, so the choice is auditable after the fact
  // rather than only visible in a channel someone had to be watching.
  const rpRows = ledgerRows(
    "SELECT payload_json FROM events WHERE ticket_id=? AND payload_json "
    + "LIKE '%risk profile override%' ORDER BY event_id DESC LIMIT 1",
    [TICKET]) || [];
  let rpPayload = null;
  try { rpPayload = JSON.parse((rpRows[0] || {}).payload_json || "null"); }
  catch (e) { rpPayload = null; }
  ok("...and the ledger records the override as an auditable event, "
     + "naming both the requested and the effective profile",
     !!rpPayload && rpPayload.requested_risk_profile === "medium"
     && rpPayload.effective_risk_profile === "medium",
     JSON.stringify(rpPayload));
  // One run only. An override is argv, never configuration.
  ok("config.json is byte-identical after the override run - an override "
     + "is one-run-only and is never written to disk",
     require("fs").readFileSync(path.join(WB, "config.json"), "utf8")
       === cfgJsonBeforeRun, path.join(WB, "config.json"));
