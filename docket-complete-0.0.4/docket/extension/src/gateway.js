...
async function run(ticketId, opts) {
  ...
  if (!hasJiraCredentials(cfg.workbench)) {
    ...
    if (pick === 'Run From File') {
      // The overrides picker's flags must SURVIVE this fallback. Until
      // 0.0.4 this re-entered with no arguments, so on a machine with no
      // Jira every gate/budget/model/risk override chosen in "Run with
      // Overrides" was silently discarded - the run looked completely
      // normal and simply ignored what was asked for. Still the REAL
      // command (never a direct runLocal() call), so the fallback keeps
      // going through the same registration everything else does.
      return vscode.commands.executeCommand('docket.runLocal', { extraArgs });
    }
    if (pick === 'Setup Help') {
...
async function runLocal(opts) {
  // Same argv plumbing as run(): plain flags spliced onto the spawn, this
  // file never reading what is IN them. `opts` arrives from two places -
  // run()'s no-Jira "Run From File" fallback, which forwards the overrides
  // picker's flags, and the plain command, which passes nothing.
  //
  // Array.isArray, not a truthiness test: VS Code hands a MENU or tree-item
  // command its context object (a Uri, a tree node) as the first argument,
  // and that must never be mistaken for an options bag and spliced into
  // python's command line.
  const extraArgs = (opts && Array.isArray(opts.extraArgs))
    ? opts.extraArgs : [];
  const out = vscode.window.createOutputChannel('Docket');
  out.show(true);
