 ...
 *     model, risk profile) that ends by calling gateway.run(undefined,
 *     {extraArgs}) - the SAME single-run gateway "Docket: Run Ticket" uses,
 *     just with the per-run flags spliced on. config.json is never touched.
 *
...
async function pickGateOffs(cfg) {
  ...
  const picked = await vscode.window.showQuickPick(items, {
    ...
    placeHolder: "Uncheck a gate to skip it for this run only (step 1 of 4)",
    canPickMany: true,
    ignoreFocusOut: true,
  });
...
async function pickBudget(cfg) {
  ...
  const raw = await vscode.window.showInputBox({
    ...
    prompt: "Budget cap for this run, in USD - leave blank to keep config.json's cap"
      + (current != null ? ` ($${current.toFixed(2)})` : "") + " (step 2 of 4)",
...
async function pickWorkerModel(cfg) {
  ...
  const pick = await vscode.window.showQuickPick(items, {
    title: OVERRIDES_TITLE,
    placeHolder: "Which model tier should the worker agents (developer/spec/qa) use this run? (step 3 of 4)",
    ignoreFocusOut: true,
  });
...
/**
 * Step 4 (optional): the per-run RISK PROFILE (Windows demo mission,
 * goal C) - the operable form of the "re-run at a deeper risk profile"
 * remedy Docket prints on a Test Spec budget stop. Automatic keeps the
 * configured/lead-derived behavior (no flag at all); Medium is the
 * recommended recovery choice - it lifts the low profile's two-request/
 * 4,000-token Test Spec restriction without disabling any gate or
 * changing acceptance criteria. Dismissing the box keeps Automatic -
 * optional steps never abort the flow (same shape as pickBudget).
 * Per-run only; config.json is never touched.
 */
async function pickRiskProfile() {
  const items = [
    { label: "Automatic",
      description: "configured/lead-derived (today's behavior)",
      value: null },
    { label: "Medium (Recommended)",
      description: "normal correction budget - lifts the low-profile Test Spec restriction",
      value: "medium" },
    { label: "High",
      description: "expanded correction/review budget",
      value: "high" },
    { label: "Low",
      description: "restricted small-ticket budget",
      value: "low" },
  ];
  const pick = await vscode.window.showQuickPick(items, {
    title: OVERRIDES_TITLE,
    placeHolder: "Risk profile for this run - budgets only, never gates or acceptance criteria (step 4 of 4)",
    ignoreFocusOut: true,
  });
  if (!pick || !pick.value) return [];
  return ["--risk-profile", pick.value];
}
...
async function runWithOverrides() {
  ...
  const riskArgs = await pickRiskProfile();
  ...
  // with these four flags spliced onto the spawn.
  return gateway.run(undefined, {
    extraArgs: [...gateOffArgs, ...budgetArgs, ...modelArgs, ...riskArgs],
  });
}
...
module.exports = {
  ...
  pickGateOffs, pickBudget, pickWorkerModel, pickRiskProfile,
