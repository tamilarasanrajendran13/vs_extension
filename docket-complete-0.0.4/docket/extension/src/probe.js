...
/**
 * Part 3 (Windows demo mission, goal B): the PROJECT-RUNTIME preflight.
 * Shells `loop.py --project-preflight-json` - which goes through the SAME
 * Python selection, environment sanitizer, containment runner and working
 * directory a real ticket uses - and folds its rows into this probe's
 * report. This is the section that would have caught the live Windows
 * failures (SYSTEMROOT stripped -> WinError 10106; PROCESSOR_* stripped ->
 * polars CPU-detection crash): an ambient-PowerShell success proves
 * nothing about the CONTAINED environment.
 */
async function probeProjectRuntime() {
  let cfg;
  try {
    cfg = await config.load();
  } catch (e) {
    add({
      name: 'PROJECT-RUNTIME preflight (part 3)', severity: 'NEEDED',
      ok: false, detail: 'no Docket config loadable: ' + e.message,
      fix: 'Open the Docket workspace folder, then re-run the probe.',
    });
    return;
  }
  if (!cfg.projectName || !cfg.projectPath) {
    add({
      name: 'PROJECT-RUNTIME preflight (part 3)', severity: 'NEEDED',
      ok: false, detail: 'no project selected yet - the project-runtime half was not checked',
      fix: "Run 'Docket: Select Project', then re-run this probe. The venv, the contained environment and the contained baseline can only be verified against a selected project.",
    });
    return;
  }
  const report = await new Promise((resolve) => {
    cp.execFile(cfg.python,
      ['loop.py', '--project-preflight-json',
       '--workbench', cfg.workbench,
       '--project', cfg.projectName,
       '--project-path', cfg.projectPath],
      // The contained baseline really runs pytest - a Spark project's
      // first boot is minutes, so this gets the same 15-minute bound
      // the pipeline's own baseline uses.
      { cwd: cfg.workbench, timeout: 15 * 60 * 1000, maxBuffer: 8 * 1024 * 1024 },
      (err, stdout, stderr) => {
        // exit 1 = an honest BLOCKED verdict, and the JSON still rides
        // stdout - parse it regardless of err.
        let parsed = null;
        try { parsed = JSON.parse(String(stdout || 'null')); } catch (e2) { /* not JSON */ }
        resolve(parsed && parsed.checks ? parsed : null);
      });
  });
  if (!report) {
    add({
      name: 'PROJECT-RUNTIME preflight (part 3)', severity: 'BLOCKER',
      ok: false, detail: 'loop.py --project-preflight-json produced no report',
      fix: 'Check the workbench path and the configured python; run it in a terminal to see the raw failure.',
    });
    return;
  }
  for (const r of report.checks) {
    add({
      name: 'PROJECT-RUNTIME ' + r.id, // one line carries section + id
      severity: r.status === 'FAIL' && r.blocking ? 'BLOCKER' : 'NEEDED',
      ok: r.status !== 'FAIL',
      detail: r.status + ': ' + r.detail,
      extra: r,
    });
  }
  add({
    name: 'PROJECT-RUNTIME verdict', severity: 'NEEDED',
    ok: report.verdict === 'READY',
    detail: report.verdict,
    fix: report.verdict === 'READY' ? undefined
      : 'Fix the failing PROJECT-RUNTIME rows above, then re-run the probe.',
  });
}
